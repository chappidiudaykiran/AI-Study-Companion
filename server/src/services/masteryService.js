...
async function updateMastery({ projectId, userId, concept, score }) {
  ...
  if (!m) {
    // First attempt counts immediately so the 2nd-failure rule fires on time.
    m = await Mastery.create({
      project: projectId, user: userId, concept, score,
      mistakes: score < 60 ? 1 : 0,
      history: [{ score, at: new Date() }],
    });
    return m;
  }
...
// Adaptive priority (NOT wrong->easy): low mastery + mistakes + per-concept recency
async function pickNextConcept({ projectId, userId, concepts }) {
  ...
  // Resolve attempt -> concept via parent Question (Attempt has no concept field).
  let conceptByQ = {};
  try {
    const qIds = [...new Set(attempts.map((a) => String(a.question)).filter(Boolean))];
    if (qIds.length) {
      const qDocs = await Question.find({ _id: { $in: qIds } }).select('_id concept').lean();
      conceptByQ = Object.fromEntries(qDocs.map((q) => [String(q._id), q.concept]));
    }
  } catch {}
  ...
  for (const c of concepts) {
    ...
    // per-concept recency: recent failures on THIS concept weigh more than global failures
    const recentMistakes = attempts.filter(
      (a) => (a.score || 0) < 60 && (conceptByQ[String(a.question)] || null) === c
    ).length;
    const globalMistakes = attempts.filter((a) => (a.score || 0) < 60).length;
    const priority = 0.4 * (100 - st.score) + 0.3 * Math.min(10, st.mistakes || 0) * 10 + 0.2 * Math.min(10, recentMistakes * 2 + globalMistakes * 0.25) * 5 + 0.1 * 5;
